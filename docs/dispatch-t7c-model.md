...
2026-09-26. Coordinator: Grok. Builder: a seat named at dispatch time. Reviewer: a different family, before merge. Depends on T7a and T7b, both merged, and on #113, which merges first. The plan row is T7 in `docs/PHASE-2.md`. The research is `docs/study-swarm/seat-instrument.dispatch.md`; numbers in parentheses are its findings.

Rewritten after the overseer's review at `ed05b61`. The first text could not be built: a frozen role cannot run, one call per session never carries feedback, and the stall's n was unset. This text is the contract. **Design review stops after round 1.** If that round blocks on gaps in the text rather than faults in the design, merging without a second round is the Director's choice. T7a took 3 rounds and T7b took 9, and this is the first slice that puts model output into the bench.
...
6. **The three small items from #115.**
   - A `--control` path outside the four roots `withoutPaths` names is not written verbatim into the record's world or `report.controls`. It is named relative to the head, or added to the paths the environment block holds.
   - The report's environment section ends the file. A line after `## Environment` is refused, by the cut and by `leaks`.
   - The comment on `separate()` says which candidates are offered to the mutants: those both trees ran soundly.
...
8. **One session per world.** `runSession` starts `feedback` empty for every session (`packages/propose/seat.js:460`), and `feedbackText` has no field for what a candidate reached. A run that made each call its own session would drop that feedback, and `callsPerSession` would never bind. A refused proposal spends no ladder budget (`packages/bench/bench.js:508`, `618`), so a client that always proposes a refusal would not stop.
   - The model is one seat session per world, of at most `callsPerSession` calls. The bench appends each call's feedback before the next: the anchors reached, the rungs, and the checker's refusal, rendered by `feedbackText`. A refused proposal costs one call against `callsPerSession` and is tallied as refused. The session stops at the role's call budget or at the ladder budget, whichever comes first.
   - The grammar still runs to its budget after the stall below. The model starts at that stall (4).
...
9. **The stall's n.** T7b's reporting grid is n of 8, 16, 32, 64, and 128. T7b leaves the choice to this slice: the model is called at the n the grid's numbers justify. The grammar does not stop at that n. It runs on to its budget, and the report's late gain is unchanged.
   - The n is the smallest grid value at which every plant below shows a late gain of 0 lines and 0 differences. If none does, n is 128, and the pull request says so.
   - Measured at seed 1, share 0.5, pitch 0.5, the sweep proposing nothing, mutants off, the room `fixtures/bench/room.json`, sweep budget 6000 quanta and 60 restores, ladder budget 12000 quanta and 120 restores. Each plant took about 10 s. A null stall means the grammar never went that many admitted candidates without a new changed line or a new difference.
     ...
     No grid value is a stall with zero late gain on every plant. n is 128, the last point on the grid. The model session starts only once that n is reached. On this run it was not reached, and a report of that run says so. The grammar still runs to its budget.
...
- `--model` without `--diff` is refused. With both, a fake client in the sweep process records one session per world, a refused proposal costs a call, and the report shows that refusal's reason.
