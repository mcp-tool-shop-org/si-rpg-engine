...
8. **One session per world.** `runSession` starts `feedback` empty for every session (`packages/propose/seat.js:460`), and `feedbackText` has no field for what a candidate reached. A run that made each call its own session would drop that feedback, and `callsPerSession` would never bind. A refused proposal spends no ladder budget (`packages/bench/bench.js:508`, `618`), so a client that always proposes a refusal would not stop.
   ...
   - The grammar keeps running to its ladder budget after the start point, and T7b's late-gain report is unchanged. The model starts at that point. Arm M is a new allocation. It is the model proposer's own budget from the start point: at most `callsPerSession` calls (or `--model-calls`), and at most one proposer's ladder budget in quanta and restores. A refused proposal costs a call and no quanta. Arm G is read from the grammar's own records. It is the grammar's candidates after the start point, in order, until their cost reaches the quanta and restores arm M spent. When the start point is the end of the grammar's budget, the bench extends the grammar past its budget by exactly arm M's spend to produce arm G. That extension counts only as arm G and changes nothing in the grammar's own report. The arms do not see each other's candidates. A finding counts as new against everything the sweep and the grammar reached before the start point.
