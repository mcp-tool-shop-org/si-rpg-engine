...
| **E2. The solver, decided and built** | E1 | The solver question from the physics consult, answered by need rather than preference. A contact inventory for a 3D RPG world: characters on ground, slopes and steps, stacked objects, doors, moving platforms. Each entry is marked "boxes on five operations can" or "cannot". If anything is "cannot", the law's step is rebuilt as one WASM binary from Rust, no relaxed SIMD, NaN refused, with its own golden under three engines. This slice starts with a study-swarm brief on deterministic solvers in Rust and WASM, cross-platform determinism in lockstep engines, and the collision shapes 2.5D traversal actually needs; the brief goes to both families before a line is written. | The inventory is in the repo. The step, in whichever language, prints one golden under three engines, and the E1 fixtures replay frame for frame under it. That replay is the migration gate for Rust. |
| **E3. Content validated at load** | E1 | A world file: bodies, colliders, and zones, where a zone is a gameplay partition above the bodies as Phase 0 says, so the tick can answer which zone a body is in. A loader refuses unknown fields, overlapping bodies, bodies inside colliders, and zones that cover nothing. Load-time hazards for content, the way verb drafts have them. The scene file from the earlier phase becomes one such world file, or is deleted. | A malformed world is refused with a reason, each case tested. A body's zone is a query the tick answers and the hash covers. |
