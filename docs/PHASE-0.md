...
**The quantum pump (2026-09-24, after the host consult).** `submit` now declares and admits and schedules; it runs no quantum. `advance` runs exactly one, whether or not anything is scheduled, so the world keeps stepping and hashing while a person does nothing. A log entry records the tick and the hash at admission, and replay advances to that tick before it submits. One action per actor: a second intent while quanta remain is refused with the count. An action's velocity clears in the advance that completes it, after that quantum is hashed. A host that throws on `draw` is detached and the quantum completes. `play`, `replay`, the hazard suite, the oracle, and the frozen seat settle after each admission, so their behavior is unchanged. The proof is `fixtures/legacy-play-log.json`, a log captured from the engine before the pump, which replays on the engine after it frame for frame: 287 frames, 6 admissions, the same final hash. The golden file is untouched, because the harness never called the tick.

**The window.** `host` is a Node process on the loopback. It calls `advance` once per timer fire, about every 16 ms, and does not catch up when the timer is late. It streams the fixture room as one JSON object per line: the colliders, then each committed frame. A page in this repo keeps the last two frames and draws boxes on a canvas. A jump larger than that quantum's velocity is drawn as a cut. Clicks, and the left and right arrows, become `move` intents. Up and down are refused: `move` sets horizontal velocity only, and a target with no horizontal delta steps to the right. A second intent while quanta remain is refused, and the page shows that reason. Nothing is queued. The process stamps the newest committed hash and ignores any hash the page sends. The timer fires every 16 ms. A quantum is 1/64 s, so the picture runs about two percent slow, and the tick's step does not change. This is the plumbing a person can look at. It is not yet a scene with something worth wanting, and the seat stays frozen.

**Phase 0 closed at `eeed37f`.** What comes next is [PHASE-1.md](PHASE-1.md): the first scene a person plays, with the harness made to prove the product, contact between bodies grown on the five operations, and the seat thawed last, on that scene only. The physics library named above is not built this phase; the first scene's contacts are boxes.
