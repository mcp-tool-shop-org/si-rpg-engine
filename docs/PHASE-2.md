...
The knowledge base then found a second defect in the same function, still present in 0.36.0: its mass ratio leaves out the pushed body's rotation, so a push off the centre overshoots, and a thin, light box launches at 41 times the pusher's speed even with F3's fix. It is filed as dimforge/rapier#1020. F5 (`docs/dispatch-f5-push-mass.md`) gives the engine's copy the effective mass at the contact point, and tightens F3's guard from 8 to 1.5 times the pusher's speed. F5 comes before F4 (the first downward cast that misses the floor by rounding, a dip of 2e-3 into the skin), because F5 stops launches.

**Decided 2026-09-25: the seat thaws as a test instrument.** The Director thawed the proposer seat as a test instrument and asked for rails so that it can take other roles later. The research is `docs/study-swarm/seat-instrument.dispatch.md`: 40 findings from five lanes, checked by the citation gate and, where an abstract could not ground a claim, on the full text by two other model families. T7 splits in two. T7a is the rails every role shares (`docs/dispatch-t7a-seat-rails.md`): a manifest per role, a role gate in the checker, provenance in the log, and a record of every model call that CI checks without a GPU. T7b is the test instrument, the first role to be thawed, after an adversarial run, and follows T6 and T7a. A role proposing for characters during play is declared in T7a and stays frozen.
