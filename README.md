...
`play` runs an intent log through the tick and prints every committed frame; `replay` reruns a log `play` wrote and fails on the first hash that differs. The model is not called by either. `load admit <draft.json>` writes a verb that passes the hazard suite; `load retire <verb>` takes it back out. `propose` asks the pinned local model in `packages/propose/model.json` for ten runs of eight proposals. Both conditions see the previous proposal and its verdict. Only one sees the checker's reason. Decoding is held to a schema whose verbs and actors come from the catalog. CI tests that loop with a stub and does not call Ollama. The seat's scene puts a pillar between the walker and a goal at x 3, y 1. The prompt has no worked examples. A run that shows the reason condition zero checker reasons is not written. The recorded run of `qwen2.5:7b`, ten seeds, budget 8, temperature 0.2, is in `fixtures/proposer-qwen2.5-7b.json`. The reason was shown 68 times. The goal was reached in 0 of 10 runs in each condition. Intents were 0 of 0 with the reason and 0 of 2 without it. Both of those intents crossed the pillar. The other proposals were beliefs and body drafts. `write-golden` rewrites `fixtures/golden.txt` after a change to the integrator or the hash function in `packages/frame/hash.js`. CI does not run it. CI installs pinned builds of V8 15.6.61, SpiderMonkey 156.0.1, and JavaScriptCore 319571 with jsvu 3.0.5, runs `harness/sim.mjs` as a module under each, and compares each to that file.
