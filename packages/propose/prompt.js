 ...
 * @param {{
 *   tick: number;
 *   bodies: Array<{ id: string, x: number, y: number, z: number, hx: number, hy: number, hz: number, qx?: number, qy?: number, qz?: number, qw?: number, wx?: number, wy?: number, wz?: number }>;
 *   episodes: string[];
 *   goal: string;
 *   obstacle: string;
...
export function proposalPrompt(view) {
  ...
  const lines = [
    ...
    'Bodies on the frame: ' + view.bodies.map((body) => {
      const qx = body.qx ?? 0;
      const qy = body.qy ?? 0;
      const qz = body.qz ?? 0;
      const qw = body.qw ?? 1;
      ...
      const wy = body.wy ?? 0;
      const wz = body.wz ?? 0;
      return body.id + ' at x ' + body.x + ' y ' + body.y + ' z ' + body.z + ', half-extents ' + body.hx + ', ' + body.hy + ', ' + body.hz + ', orientation ' + qx + ' ' + qy + ' ' + qz + ' ' + qw + ', angular ' + wx + ' ' + wy + ' ' + wz;
    }).join('; ') + '.',
