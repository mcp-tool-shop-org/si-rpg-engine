...
| Objets avec position, vitesse, un quaternion canonique, vitesse angulaire et demi-étendues ; les boîtes dynamiques tournent ; un personnage cinématique avec un pas automatique de 0,3, une ascension de 45° et un ajustement de 0,2 ; le sommeil est compté en pas. | `solver/src/rapier_law.rs`, `packages/tick/world.js` | `fixtures/behavior-3d.json`, `behavior-rotation.json`, `behavior-ramp.json`, `shape-traversal.json` |
| Fichiers de monde : objets, collisionneurs statiques orientés, champs de hauteur, zones en tant que partition, douze refus de chargement, dangers au chargement et un index auquel l'hôte fait confiance ; les actions se tiennent sur la même surface de terrain à deux triangles que les collisionneurs physiques. | `packages/tick/scene.js`, `packages/tick/admit-world.js`, `worlds/` | `packages/tick/scene.test.js`, `harness/surface.test.js` |
...
| Une trace de chaque pas en bits exacts, et un outil qui indique le premier pas, l'objet et le champ où deux séquences divergent. | `harness/trace.mjs`, `harness/first-difference.js` | `harness/trace.test.js` ; l'intégration continue affiche la première différence lorsqu'un moteur s'écarte de la version de référence. |
...
| Sauvegarde et restauration de deux manières, en rejouant les entrées jusqu'à un pas ou en copiant la mémoire du module physique, chacune étant prouvée pour continuer exactement. | `packages/tick/runs.js`, `solver/build.mjs` | `harness/restore.test.js` |
...
| Tests de ce que le monde a fait : un parcours de personnage aux limites mesurées du contrôleur, un objet mince et rapide contre un mur mince et des joints de terrain. | `harness/course.test.js`, `harness/outcome.test.js` | `write-golden` refuse d'écrire tant que l'un d'eux échoue. |
...
241 tests, sept éléments de test de comportement qui rejouent pas à pas, et deux hachages de référence affichés par trois moteurs sur x64 et par node sur ARM64, à chaque validation.
...
`npm test` construit d'abord la physique et analyse le fichier binaire. Sous Linux, la construction est comparée au hachage enregistré ; sur un autre hôte, elle affiche son propre hachage, car la construction Linux est l'artefact enregistré.
...
Lorsque deux séquences sont en désaccord, la trace indique où :
...
La vue de débogage est une vue de débogage. Elle affiche les images validées sous forme de boîtes projetées le long de l’axe choisi avec `x`, `y` ou `z` ; un clic correspond à une cible sur le plan du sol ; `M`, `C`, `G`, `D` et `U` permettent de choisir les actions de déplacement, d’escalade, de ramassage, de dépôt et d’utilisation ; la zone du personnage et les convictions de chaque entité sont affichées à côté du compteur et du hachage. Elle n’affiche jamais rien que le compteur ne contient pas.
...
Un compteur initialisé est la loi. Une étape, un quantum, est de 1/64 s ; chaque étape est hachée, et l’action d’un personnage s’étend sur plusieurs étapes. La relecture est la graine plus le journal de ce qui a été accepté. Le modèle propose des intentions, des convictions typées et des ébauches de corps ; le vérificateur pour cette classe les accepte ou les refuse. Les ébauches d’actions et les fichiers de monde attendent jusqu’au moment du chargement et passent une série de tests. L’hôte reçoit les images validées et renvoie les intentions. La présentation n’a aucun moyen de revenir au hachage.

## Modèle de confiance

Le moteur s’exécute localement et n’accède qu’aux fichiers situés dans son propre répertoire : mondes, ébauches d’actions, éléments graphiques et tout journal que vous demandez à une commande d’écrire. `host` lie uniquement `127.0.0.1`. Aucune commande n’ouvre d’autre socket ; l’instrument `propose` gelé, une fois qu’une personne le débloque, communique avec un serveur Ollama local et nulle part ailleurs. Aucune information d’identification n’est lue, stockée ou envoyée. Aucune télémétrie n’est collectée. Le contenu créé est considéré comme non fiable et est validé au moment du chargement ; un fichier refusé ne change rien. Le fichier binaire WebAssembly est généré à partir du code source dans l’environnement CI et est verrouillé par son SHA-256, et n’est jamais enregistré sous forme d’octets. Sa mémoire est fixée à 32 MiB et ne peut pas augmenter, de sorte qu’un monde trop dense pour lui se bloque de la même manière sur chaque hôte au lieu de diverger. Voir [SECURITY.md](SECURITY.md).

## État du support

Version antérieure à 1.0, publiée sous le nom de `0.x` à partir de `main`. Il n’y a aucune garantie de compatibilité entre les versions ; chaque modification de la loi hachée est enregistrée dans [CHANGELOG.md](CHANGELOG.md) avec le hachage correspondant. Testé sur Node 22 et Rust 1.98.1 sur Ubuntu x64 et ARM64 dans l’environnement CI, et compilé quotidiennement sur Windows 11.
