...
| Rôles pour les sièges du modèle : un manifeste par rôle, la règle des deux dérivée de ce que le rôle lit, une porte de rôle dans le vérificateur, une provenance sur chaque admission, des étiquettes de confiance qui restent avec une croyance et chaque appel de modèle enregistré et vérifié sans GPU ; les deux rôles déclarés sont figés. | `predicates/roles/`, `packages/tick/roles.js`, `packages/tick/gate.js`, `packages/propose` | `packages/tick/gate.test.js`, `packages/propose/record.test.js` au cours des sessions dans `fixtures/sessions/` |
| Relancer à partir d’une graine et d’un journal, et afficher une vue de débogage de l’itération sur localhost. | `packages/tick/replay.js`, `packages/host` | `fixtures/first-scene-played.json` est la simulation d’une personne à travers la limite de l’hôte. |

378 tests, sept scénarios de comportement qui relancent étape par étape, et deux hachages de référence imprimés par trois moteurs sur x64 et par Node sur ARM64, à chaque validation.

## Installation

Prérequis : Node 20 ou version ultérieure, et la chaîne d’outils Rust avec la cible `wasm32-unknown-unknown` pour la compilation de la physique. CI fixe Rust à la version 1.98.1 ; `rustup target add wasm32-unknown-unknown` est l’étape supplémentaire après l’installation de rustup.
...
`npm test` compile la physique et effectue d’abord une analyse du code binaire. Sur Linux, la compilation est comparée au hachage fixe ; sur un autre hôte, elle signale son propre hachage, car la compilation Linux est l’artefact fixe.
