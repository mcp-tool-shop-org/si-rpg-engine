...
| Um binário em duas arquiteturas de CPU, com memória fixada em 32 MiB e uma verificação que rejeita instruções escolhidas pelo host, crescimento de memória e estado mantido fora da memória. | `solver/build.rs`, `solver/src/arena.rs`, `solver/lint.mjs` | O trabalho ARM64 do CI; `solver/lint.test.js`, `harness/caps.test.js`. |
...
| Reproduza a partir de uma semente e de um arquivo de log, e visualize o estado de depuração do ciclo no localhost. | `packages/tick/replay.js`, `packages/host` | `fixtures/first-scene-played.json` é a execução de uma pessoa através da fronteira do host. |

363 testes, sete conjuntos de testes de comportamento que reproduzem passo a passo, e dois hashes de referência impressos por três mecanismos em x64 e pelo Node em ARM64, em cada commit.
...
`npm test` compila a física e, em seguida, verifica o código binário. No Linux, a construção é comparada com o hash fixo; em outro host, ele relata seu próprio hash, porque a construção do Linux é o artefato fixo.
...
Um mundo é restaurado de três maneiras, e nenhum escreve no estado interno do motor de física, que é o motivo pelo qual cada um é exato. Você pode reproduzir suas entradas aceitas em um determinado passo. Você pode copiar toda a memória do módulo de física com `imageSolver()` e restaurá-la com `restoreImage()`. Ou você pode salvar todo o ciclo com `save()` e restaurá-lo com `restore(saved)`, o que não requer reprodução e é como a simulação retorna a um estado milhares de vezes. Uma imagem de um binário diferente, com um comprimento incorreto ou com um byte alterado, é recusada, e uma cópia de segurança que não verifica todas as alterações não altera nada.

A visualização de depuração é uma visualização de depuração. Ela desenha os quadros confirmados como caixas projetadas ao longo do eixo escolhido com `x`, `y` ou `z`; um clique é um alvo no plano de solo; `M`, `C`, `G`, `D` e `U` selecionam mover, escalar, pegar, soltar e usar; a zona do personagem e as crenças de cada agente ficam ao lado do ciclo e do hash. Ela nunca desenha nada que o ciclo não contenha.
