...
| Uma varredura de alcance quando um mundo é admitido: seus estados alcançáveis são explorados com as ações admitidas, através do verificador, a partir dos salvamentos do próprio passo. Uma zona que nada alcança, um corpo carregado para fora do mundo ou um lançamento rejeita o mundo, com uma testemunha que `replay` reproduz. | `packages/load/sweep.js` | `harness/sweep.test.js`, os ambientes de teste fechados em `fixtures/sweep/`. |
...
352 testes, sete conjuntos de testes de comportamento que reproduzem passo a passo e dois hashes de referência impressos por três mecanismos em x64 e por node em ARM64, em cada commit.

## Instalar

Requisitos: Node 20 ou versão mais recente e a cadeia de ferramentas Rust com o alvo `wasm32-unknown-unknown` para a construção da física. O CI fixa o Rust na versão 1.98.1; `rustup target add wasm32-unknown-unknown` é o passo extra após a instalação do rustup.
...
`npm test` compila a física e, em seguida, verifica o binário. No Linux, a construção é comparada com o hash fixo; em outro host, ele relata seu próprio hash, porque a construção do Linux é o artefato fixo.

## Uso

Cada comando é executado a partir de qualquer diretório, responde `--help`, sai com código 0 em caso de sucesso, 1 com uma razão em caso de recusa e 2 em caso de erro de uso ou falha inesperada. `--debug` permite que um rastreamento de pilha seja exibido.
...
Um mundo é restaurado de três maneiras, e nenhum escreve no estado interno do motor de física, razão pela qual cada um é exato. Você pode reproduzir suas entradas aceitas em uma etapa. Você pode copiar toda a memória do módulo de física com `imageSolver()` e restaurá-la com `restoreImage()`. Ou você pode salvar toda a iteração com `save()` e restaurá-la com `restore(saved)`, o que não requer reprodução e é como a simulação retorna a um estado milhares de vezes. Uma imagem de outro binário, com o comprimento incorreto ou com um byte alterado, é recusada, e uma cópia de segurança que não verifica todas as alterações não altera nada.

A visualização de depuração é uma visualização de depuração. Ela desenha os quadros confirmados como caixas projetadas ao longo do eixo escolhido com `x`, `y` ou `z`; um clique é um alvo do plano de solo; `M`, `C`, `G`, `D` e `U` selecionam mover, escalar, pegar, soltar e usar; a zona do personagem e as crenças de cada mente ficam ao lado da iteração e do hash. Ela nunca desenha nada que a iteração não contenha.

Um arquivo de mundo é JSON: `name`, `seed`, `bodies`, `colliders`, `zones` e, opcionalmente, `heightfield` e `goal`. Um corpo é `{ id, x, y, z, vx, vy, vz, hx, hy, hz }` com um quatérnio e uma velocidade angular opcionais; um colisor estático é uma caixa definida por seus limites com um quatérnio opcional em relação ao seu centro; uma zona é uma caixa nomeada. Campos desconhecidos, IDs duplicados, corpos sobrepostos, um corpo dentro de um colisor, um quatérnio não unitário, uma zona degenerada ou inacessível e um objetivo que não nomeia nada são recusados com uma razão.

## A lei, em uma frase

Uma iteração com semente é a lei. Um passo, um quantum, é 1/64 s; cada passo é calculado e a ação de um personagem abrange muitos passos. A reprodução é a semente mais o registro do que foi admitido. O modelo propõe intenções, crenças tipadas e rascunhos de corpo; o verificador para essa classe admite ou recusa. Os rascunhos de ação e os arquivos de mundo aguardam até o tempo de carregamento e passam por um conjunto de testes de segurança. O host recebe os quadros confirmados e retorna as intenções. A apresentação não tem caminho de volta para o hash.

## Modelo de confiança

O motor é executado localmente e acessa apenas arquivos dentro de seu próprio diretório de checkout: mundos, rascunhos de ação, acessórios e qualquer registro que você peça a um comando para gravar. `host` vincula `127.0.0.1` apenas. Nenhum comando abre nenhum outro socket além de `propose`, que se comunica com um servidor Ollama local e em nenhum outro lugar, e apenas para uma função cujo manifesto está desbloqueado para atuar em um mundo de teste; ambas as funções que o motor declara estão congeladas, portanto, ele recusa antes que qualquer cliente de modelo seja carregado. A proposta de um modelo entra no mundo apenas através do portão de função, que a mantém fiel ao manifesto de sua função. Nenhuma credencial é lida, armazenada ou enviada. Nenhum telemetria é coletado. O conteúdo criado é não confiável e é validado no carregamento; um arquivo recusado não altera nada. O binário WebAssembly é construído a partir do código-fonte no CI e fixado por seu SHA-256, nunca confirmado como bytes. Sua memória é fixa em 32 MiB e não pode aumentar, portanto, um mundo muito denso para ele para de funcionar da mesma forma em todos os hosts, em vez de divergir. Consulte [SECURITY.md](SECURITY.md).

## Status de suporte

Pré-1.0, lançado como `0.x` a partir de `main`. Não há promessa de compatibilidade entre as versões; cada alteração na lei calculada é registrada em [CHANGELOG.md](CHANGELOG.md) com o hash dourado que ela produziu. Testado no Node 22 e no Rust 1.98.1 no Ubuntu x64 e ARM64 no CI, e construído diariamente no Windows 11.

## Licença

MIT, exceto `solver/src/kcc.rs`, uma cópia modificada de parte do controlador de personagem do Rapier, que está sob a Licença Apache 2.0 (`solver/LICENSE-APACHE-2.0`, `solver/NOTICE`). Construído por <a href="https://mcp-tool-shop.github.io/">MCP Tool Shop</a>.
