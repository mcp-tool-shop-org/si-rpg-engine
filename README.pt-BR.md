...
| Arquivos de mundo: corpos, colisores estáticos orientados, campos de altura, zonas como uma partição, doze recusas de carregamento, perigos no carregamento e um índice no qual o host confia; as ações se encontram na mesma superfície de terreno de dois triângulos com a qual a física colide. | `packages/tick/scene.js`, `packages/tick/admit-world.js`, `worlds/` | `packages/tick/scene.test.js`, `harness/surface.test.js` |
...
| Testes do que o mundo fez: um percurso de personagem nos limites medidos do controlador, um corpo fino e rápido contra uma parede fina e costuras de terreno. | `harness/course.test.js`, `harness/outcome.test.js` | `write-golden` se recusa a gravar enquanto algum deles falhar. |
| Reprodução a partir de uma semente e um log, e uma visualização de depuração do passo em localhost. | `packages/tick/replay.js`, `packages/host` | `fixtures/first-scene-played.json` é a reprodução de uma pessoa através da fronteira do host. |

241 testes, sete conjuntos de testes de comportamento que reproduzem passo a passo e dois hashes padrão impressos por três mecanismos em x64 e por node em ARM64, em cada commit.
...
A visualização de depuração é uma visualização de depuração. Ela desenha os quadros confirmados como caixas projetadas ao longo do eixo escolhido com `x`, `y` ou `z`; um clique é um alvo no plano de referência; `M`, `C`, `G`, `D` e `U` selecionam mover, escalar, pegar, soltar e usar; a zona do personagem e as crenças de cada um ficam ao lado do contador e do hash. Ela nunca desenha nada que o contador não contenha.

Um arquivo de mundo é JSON: `name`, `seed`, `bodies`, `colliders`, `zones` e, opcionalmente, `heightfield` e `goal`. Um corpo é `{ id, x, y, z, vx, vy, vz, hx, hy, hz }` com um quatérnio e uma velocidade angular opcionais; um colisor estático é uma caixa definida por seus limites, com um quatérnio opcional em relação ao seu centro; uma zona é uma caixa com nome. Campos desconhecidos, IDs duplicados, corpos sobrepostos, um corpo dentro de um colisor, um quatérnio não unitário, uma zona degenerada ou inacessível e um objetivo que não nomeia nada são rejeitados com uma justificativa.

## A lei, em uma única frase

Um contador com semente é a lei. Um passo, um quantum, é de 1/64 s; cada passo é calculado por hash, e a ação de um personagem abrange vários passos. A reprodução é a semente mais o registro do que foi aceito. O modelo propõe intenções, crenças tipadas e esboços de corpos; o verificador para essa classe aceita ou rejeita. Os esboços de ação e os arquivos de mundo aguardam até o momento do carregamento e passam por um conjunto de testes. O host recebe os quadros confirmados e retorna as intenções. A apresentação não tem caminho de volta para o hash.

## Modelo de confiança

O motor é executado localmente e acessa apenas arquivos dentro de seu próprio diretório: mundos, esboços de ação, elementos e qualquer registro que você peça a um comando para gravar. `host` vincula apenas `127.0.0.1`. Nenhum comando abre qualquer outro socket; o instrumento `propose` congelado, quando uma pessoa o descongelar, se comunica com um servidor Ollama local e em nenhum outro lugar. Nenhuma credencial é lida, armazenada ou enviada. Nenhum dado de telemetria é coletado. O conteúdo criado é não confiável e é validado no momento do carregamento; um arquivo rejeitado não altera nada. O arquivo binário WebAssembly é construído a partir do código-fonte no CI e fixado por seu SHA-256, nunca sendo confirmado como bytes. Sua memória é fixa em 32 MiB e não pode aumentar, portanto, um mundo muito denso para ele interrompe da mesma forma em cada host, em vez de divergir. Consulte [SECURITY.md](SECURITY.md).

## Status de suporte

Pré-1.0, lançado como `0.x` a partir de `main`. Não há garantia de compatibilidade entre as versões; cada alteração na lei calculada por hash é registrada em [CHANGELOG.md](CHANGELOG.md) com o hash dourado que ela produziu. Testado no Node 22 e no Rust 1.98.1 no Ubuntu x64 e ARM64 no CI, e construído diariamente no Windows 11.
