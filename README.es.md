...
Lo que pretende ser es el núcleo de simulación dentro de un host: un navegador, Godot o Unreal dibujan la imagen y envían entradas, mientras que la física, la huella y el registro permanecen aquí. El trabajo actual es el conjunto de pruebas que necesita un motor listo para su distribución, y la mayor parte de ello se encuentra en: un rastreo que identifica el primer paso y el valor en el que dos ejecuciones divergen, una función de guardar y restaurar que se ha demostrado que es exacta, una segunda arquitectura de CPU, un análisis del código de la física compilada y pruebas que verifican lo que hizo el mundo en lugar de solo su huella. Después del conjunto de pruebas, se implementan las colisiones a partir de mallas y una vinculación al host. El diseño y los planes se encuentran en [docs/PHASE-0.md](docs/PHASE-0.md), [docs/PHASE-1.md](docs/PHASE-1.md) y [docs/PHASE-2.md](docs/PHASE-2.md).

## Qué se está construyendo

| Capacidad. | Dónde. | Prueba. |
|---|---|---|
| Un paso de tiempo fijo; el estado de cada paso se calcula con un hash FNV-1a de dos carriles sobre cada valor f64; se rechaza NaN; el cero con signo se normaliza. | `packages/tick`, `packages/frame` | `fixtures/golden-arith.txt` ha leído `0d38671370d12d1e` desde la primera ejecución. |
| La ley de la física en Rust en `rapier3d-f64` con `enhanced-determinism`, un único archivo binario de WebAssembly con su resumen de Linux fijado. | `solver/` | `fixtures/solver.sha256`; CI reconstruye y compara. |
...
| Un rastreo de cada paso en bits exactos, y una herramienta que identifica el primer paso, el cuerpo y el campo en el que dos ejecuciones divergen. | `harness/trace.mjs`, `harness/first-difference.js` | `harness/trace.test.js`; CI imprime la primera diferencia cuando un motor se desvía de la versión "dorada". |
| Números de comportamiento junto con la versión "dorada": el paso de inactividad y la posición final de cada cuerpo, la zona del caminante y la longitud y el resumen de la instantánea. | `fixtures/golden-behaviour.json`, `harness/check.js` | `harness/check.test.js` |
| Guardar y restaurar de dos maneras, ya sea reproduciendo las entradas de un paso o copiando la memoria del módulo de física, y se ha demostrado que ambas opciones continúan exactamente. | `harness/replay-to.mjs`, `solver/build.mjs` | `harness/restore.test.js` |
...
174 pruebas, siete conjuntos de pruebas de comportamiento que reproducen paso a paso y dos hashes "dorados" impresos por tres motores en x64 y por node en ARM64, en cada confirmación.
...
`npm test` construye la física y analiza el archivo binario primero. En Linux, la construcción se compara con el resumen fijado; en otro host, informa de su propio resumen, porque la construcción de Linux es el artefacto fijado.

## Uso

Cada comando se ejecuta desde cualquier directorio, devuelve `--help`, sale con 0 en caso de éxito, 1 con una razón en caso de rechazo y 2 en caso de error de uso o un fallo inesperado. `--debug` permite que se muestre un rastreo de la pila.
...
Un mundo se restaura de dos maneras, y ninguna de ellas escribe en el estado interno del motor de física, por lo que ambas son exactas: reproduce sus entradas aceptadas en un paso o copia toda la memoria del módulo de física con `imageSolver()` y la vuelve a colocar con `restoreImage()`. Se rechaza una imagen de otro archivo binario, de la longitud incorrecta o con un byte cambiado.

La vista de depuración es una vista de depuración. Dibuja los fotogramas confirmados como cajas proyectadas a lo largo del eje elegido con `x`, `y` o `z`; un clic es un objetivo en el plano base; `M`, `C`, `G`, `D` y `U` eligen moverse, escalar, recoger, soltar y usar; la zona del personaje y las creencias de cada entidad se sitúan junto a la marca de tiempo y el código hash. Nunca dibuja nada que la marca de tiempo no contenga.
...
Una marca de tiempo con semilla es la ley. Un paso, un cuanto, es de 1/64 de segundo; cada paso se calcula con un código hash y la acción de un personaje abarca muchos pasos. La reproducción es la semilla más el registro de lo que se admitió. El modelo propone intenciones, creencias tipificadas y borradores del cuerpo; el verificador para esa clase los admite o los rechaza. Los borradores de acción y los archivos de mundo esperan hasta el momento de la carga y pasan por una serie de pruebas. El host recibe los fotogramas confirmados y devuelve las intenciones. La presentación no tiene ninguna vía de retorno al código hash.

## Modelo de confianza

El motor se ejecuta localmente y solo accede a los archivos dentro de su propio directorio de trabajo: mundos, borradores de acción, archivos de configuración y cualquier registro que se le pida a un comando que escriba. `host` vincula solo a `127.0.0.1`. Ningún comando abre ningún otro socket; el instrumento `propose` congelado, una vez que una persona lo descongela, se comunica con un servidor Ollama local y en ningún otro lugar. No se leen, almacenan ni envían credenciales. No se recopila ninguna telemetría. El contenido creado por el usuario no es de confianza y se valida en el momento de la carga; un archivo rechazado no cambia nada. El archivo binario de WebAssembly se crea a partir del código fuente en CI y se fija mediante su SHA-256, y nunca se guarda como bytes. Su memoria está fija en 32 MiB y no puede crecer, por lo que un mundo demasiado denso para él se detiene de la misma manera en cada host en lugar de divergir. Consulte [SECURITY.md](SECURITY.md).

## Estado de soporte

Pre-1.0, lanzado como `0.x` desde `main`. No hay ninguna promesa de compatibilidad entre las versiones; cada cambio en la ley con código hash se registra en [CHANGELOG.md](CHANGELOG.md) junto con el código hash resultante. Probado en Node 22 y Rust 1.98.1 en Ubuntu x64 y ARM64 en CI, y compilado diariamente en Windows 11.
