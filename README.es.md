...
| La ley de la física en Rust en `rapier3d-f64` con `enhanced-determinism`, un único archivo binario de WebAssembly con su resumen de Linux fijado; un mundo de física en ejecución, reconstruido solo cuando cambia la geometría, con un cuerpo que se cambia en su lugar cuando comienza o termina una acción. | `solver/` | `fixtures/solver.sha256`, que CI reconstruye y compara; `harness/switch.test.js`. |
...
356 pruebas, siete conjuntos de pruebas de comportamiento que reproducen cada paso, y dos hashes de referencia generados por tres motores en x64 y por Node en ARM64, en cada confirmación.
...
`npm test` compila la física y realiza un análisis estático del binario. En Linux, la compilación se compara con el hash almacenado; en otro sistema, informa su propio hash, ya que la compilación de Linux es el artefacto almacenado.
...
Cuando dos ejecuciones no coinciden, el registro indica dónde.
...
Un mundo se restaura de tres maneras, y ninguna modifica el estado interno del motor de física, por lo que cada una es exacta. Puedes reproducir sus entradas aceptadas en un paso. Puedes copiar toda la memoria del módulo de física con `imageSolver()` y volver a colocarla con `restoreImage()`. O puedes guardar todo el ciclo con `save()` y volver a colocarlo con `restore(saved)`, lo que no requiere reproducción y es la forma en que la simulación vuelve a un estado miles de veces. Una imagen de otro binario, de la longitud incorrecta o con un byte modificado, se rechaza, y un archivo guardado que no verifica todos los cambios no altera nada.

La vista de depuración es una vista de depuración. Dibuja los fotogramas confirmados como cajas proyectadas a lo largo del eje elegido con `x`, `y` o `z`; un clic es un objetivo en el plano base; `M`, `C`, `G`, `D` y `U` seleccionan moverse, escalar, recoger, soltar y usar; la zona del personaje y las creencias de cada agente se muestran junto al ciclo y el hash. Nunca dibuja nada que el ciclo no contenga.
...
El motor se ejecuta localmente y solo accede a los archivos dentro de su propio directorio de trabajo: mundos, borradores de acción, conjuntos de pruebas y cualquier registro que un comando deba escribir. `host` vincula `127.0.0.1` únicamente. Ningún comando abre ningún otro socket que no sea `propose`, que se comunica con un servidor Ollama local y en ningún otro lugar, y solo para un rol cuyo manifiesto está descongelado para actuar en un mundo de prueba; ambos roles que el motor declara están congelados, por lo que rechaza antes de que se cargue cualquier cliente de modelo. La propuesta de un modelo entra en el mundo solo a través de la puerta del rol, que lo limita a su manifiesto de rol. No se leen, almacenan ni envían credenciales. No se recopila ninguna telemetría. El contenido creado por el usuario no es de confianza y se valida en el momento de la carga; un archivo rechazado no cambia nada. El binario WebAssembly se compila a partir del código fuente en CI y se fija mediante su SHA-256, y nunca se confirma como bytes. Su memoria está fija en 32 MiB y no puede crecer, por lo que un mundo demasiado denso para él se detiene de la misma manera en cada host en lugar de divergir. Consulte [SECURITY.md](SECURITY.md).

## Estado de soporte

Pre-1.0, lanzado como `0.x` desde `main`. No hay garantía de compatibilidad entre las versiones; cada cambio en la ley calculada se registra en [CHANGELOG.md](CHANGELOG.md) junto con el hash de referencia que produjo. Probado en Node 22 y Rust 1.98.1 en Ubuntu x64 y ARM64 en CI, y compilado diariamente en Windows 11.
