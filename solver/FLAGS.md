...
These are the builder's decisions for questions 5 and 6 in `docs/rust-kb-requests.md`, taken from the toolchain before `docs/rust-kb-answers.md` existed. `-C target-feature=-relaxed-simd` alone does not guarantee the 1.98.1 backend emits none: a per-function `target_feature` overrides it, as measured above. The lint's scan is the guarantee, and `solver/lint.test.js` plants every opcode from `0x100` through `0x113`, `f64x2.relaxed_madd` among them, to prove it catches each one.
