...
F4 adds a second change, in `move_shape`: when the move's first cast finds nothing while the character was grounded at its start, the copy casts once more with the skin 1e-9 larger. A character standing on a floor starts its move with its dilated shape on the floor's face to within rounding, and there parry's GJK cast can lose its search direction to rounding and report no hit (https://github.com/dimforge/parry/issues/452), so the character took its whole move, gravity step included, 1.953125e-3 into its 0.01 skin: 8 of the product walker's 10,000 flat-ground quanta at the origin and 7 at 1e6. With the skin 1e-9 larger the start is clearly inside the dilated shape, and parry's answer for an impact at the start keeps the hit only if the move approaches the collider. The retry is the const parameter `RETRY`, on in the law and off only in the tests. parry 0.31.1, which rapier 0.36.0 brings, has the same GJK.
...
2. Run the control tests first, `cargo test --release --locked` in `solver/`:
   - `the_copy_with_its_branch_off_moves_the_character_as_rapiers_controller_does_bit_for_bit` calls Rapier's own controller and the copy with both changes off with the same inputs at every quantum of the flat walk at the origin and at 1e6, the ten course cases, the 0.29 step from 20 starts in four directions, and the verb fixture's capsule carry, and at zero desired translation, and requires every movement to match bit for bit. A copy that parts from Rapier fails it with the run, the quantum, and both movements.
   - `with_the_retry_on_the_law_parts_from_the_law_before_f4_only_where_the_retry_hits_and_each_hit_starts_on_the_skin` calls the law's copy and the copy with the retry off with the same inputs at every quantum of the law's runs of the flat walks, the course, the step from 80 starts, every law run, and the product scene at 1e6, and requires the two to part only on a call where the retry fired and hit, each hit starting within 2.5e-15 of the skin.
   - `the_copy_with_its_retry_off_moves_the_character_as_the_law_did_before_f4_bit_for_bit` holds the copy with the retry off to digests of the law before F4 at rapier3d-f64 0.35.3. A bump that moves those runs fails it by design; once the control test against Rapier passes, the digests are recorded again from the bumped law, and the commit says why.
3. Then the course and the outcome tests, and only then may a golden move.

If upstream has fixed the degenerate case, the branch and the copy can go together, and the law calls Rapier's routine again. If parry fixes #452, the retry can go, with its control tests.
