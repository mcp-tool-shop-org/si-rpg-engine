...
| `../review.mjs` | The external review: the dispatch, the diff, CI's lines, and the coordinator's evidence, sent to the review panel (`../panel.js`), with a receipt and a summary for the pull request. A pull request too large for one message is reviewed in passes over disjoint files, one `--files <regex>` each; a dry run (`--dry-run`) shows whether a pass fits. | After the verification. The runner's own header gives its usage and its rules. |
