...
- **The instrument's bench.** `bench` takes a change and the build before it, and runs each tree in a process of its own. It names what the change touches, proposes inputs with the sweep and a seeded grammar, and runs every candidate on both trees. It reports what reached the change, where the two runs first part, and what fails only on the change, and every verdict comes from the engine, none from a model. When `solver/` changes, the law's reach is read from a coverage build whose run matches the product build's frame for frame, and mutants planted at the change measure the bench. F2's fix, planted with its branch off, is found at quantum 98. No workflow runs it, and it calls no model.
...
- **The product golden is `69a671f962665563`.** The product scene's climber now stands on a floor, where before it fell for the whole run; an action's start or end no longer rebuilds the physics world; the walker no longer loses a step's travel on flat ground; and no step sinks into the floor.
...
- **No step sinks into the floor.** On a few steps the character's first cast of its move found no floor although it stood on one, because parry's cast loses its direction to rounding when it starts exactly on the contact skin (dimforge/parry#452, open upstream). The character dropped a step of gravity into its skin and climbed back over 20 steps. The engine's copy of the controller now casts once more with the skin 1e-9 larger when that happens, and a native test shows the copy without it moves as before, bit for bit. The product golden moves with it.
- **A push sizes each contact's impulse by the pushed body's effective mass.** Rapier's routine sizes it from the two bodies' linear masses only, leaving out the pushed body's turning. So a push off the centre overshot, and a thin, light box launched at 41 times its pusher's speed (dimforge/rapier#1020, open upstream). The engine's copy of the routine now uses the effective mass at each contact point. The push guard tightens to one and a half times the speed of the character that pushed. The sweep no longer finds three bodies a push carried over an open edge.
...
- **A push moves a body only at its own contact points.** When two dynamic bodies were near the character, Rapier 0.35.3's impulse routine could push one at the other's contact points and launch it (dimforge/rapier#1004, fixed in Rapier 0.36.0). The character now pushes through the engine's own copy of that routine, `solver/src/impulses.rs`, with Rapier's fix backported, and a native test shows the copy without the fix pushes exactly as Rapier's routine does. A guard fails any push that leaves a body faster than a stated multiple of its pusher's speed. Red room A (`fixtures/push/red-room-a.json`), where the old routine launched a crate at 26 units per second, now stays under it. The sweep had refused the minds fixture's world because a crate launched out of it; that world is now admitted. The copy is under the Apache License 2.0.
