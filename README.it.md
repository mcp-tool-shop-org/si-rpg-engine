...
| La legge della fisica in Rust su `rapier3d-f64` con `enhanced-determinism`, un singolo file binario WebAssembly con il suo hash Linux memorizzato; un mondo fisico in esecuzione, ricostruito solo quando la geometria cambia, con un corpo sostituito al suo posto quando un'azione inizia o termina | `solver/` | `fixtures/solver.sha256`, che CI ricostruisce e confronta; `harness/switch.test.js` |
| Corpi con posizione, velocità, un quaternione canonico, velocità angolare e metà estensioni; i box dinamici ruotano; un personaggio cinematico con un autostep di 0,3, una salita di 45° e uno snap di 0,2; il sonno viene contato in passaggi | `solver/src/rapier_law.rs`, `packages/tick/world.js` | `fixtures/behavior-3d.json`, `behavior-rotation.json`, `behavior-ramp.json`, `shape-traversal.json` |
...
| Una scansione di raggiungibilità quando un mondo viene ammesso: i suoi stati raggiungibili vengono esplorati con le azioni ammesse, attraverso il checker, a partire dai salvataggi del ciclo stesso. Una zona che nulla raggiunge, un corpo trasportato fuori dal mondo o un lancio rifiutano il mondo, con un testimone che `replay` riproduce | `packages/load/sweep.js` | `harness/sweep.test.js`, le stanze di test chiuse in `fixtures/sweep/` |
...
| Numeri di comportamento accanto al valore di riferimento: il passaggio di sonno e la posizione finale di ogni corpo, la zona del camminatore e la lunghezza e l'hash dello snapshot | `fixtures/golden-behaviour.json`, `harness/check.js` | `harness/check.test.js` |
| Salvataggio e ripristino in tre modi: riproducendo gli input di un passaggio, copiando la memoria del modulo di fisica o utilizzando il salvataggio dello stato completo del ciclo, che ripristina senza riproduzione; ciascuno di essi si dimostra in grado di continuare esattamente | `packages/tick/runs.js`, `packages/tick/tick.js`, `solver/build.mjs` | `harness/restore.test.js` |
...
| Ruoli per i modelli di simulazione: un manifesto per ogni ruolo, la "Regola dei Due" derivata da ciò che il ruolo implica, un "gate" di ruolo nel checker, informazioni sull'origine per ogni ammissione, etichette di affidabilità che rimangono associate a una credenza e ogni chiamata al modello viene registrata e verificata senza l'uso di una GPU; entrambi i ruoli dichiarati sono fissi. | `predicates/roles/`, `packages/tick/roles.js`, `packages/tick/gate.js`, `packages/propose` | `packages/tick/gate.test.js`, `packages/propose/record.test.js` durante le sessioni in `fixtures/sessions/` |
| Riproduzione da un seme e un log, e una visualizzazione di debug del "tick" su localhost. | `packages/tick/replay.js`, `packages/host` | `fixtures/first-scene-played.json` rappresenta l'interazione di una persona all'interno dei limiti dell'ambiente di simulazione. |

380 test, sette "fixture" di comportamento che riproducono passo dopo passo, e due "hash" di riferimento stampati da tre motori su x64 e da Node su ARM64, ad ogni commit.

## Installazione

Requisiti: Node 20 o versione successiva, e la toolchain Rust con il target `wasm32-unknown-unknown` per la compilazione della fisica. Il sistema di Continuous Integration (CI) utilizza Rust 1.98.1; `rustup target add wasm32-unknown-unknown` è l'unico passaggio aggiuntivo dopo l'installazione di rustup.
...
`npm test` compila la fisica e verifica il codice binario. Su Linux, la compilazione viene confrontata con l'hash memorizzato; su un altro sistema, segnala il proprio hash, poiché la compilazione su Linux è l'artefatto memorizzato.

## Utilizzo

Ogni comando viene eseguito da qualsiasi directory, risponde a `--help`, termina con codice 0 in caso di successo, 1 con una motivazione in caso di rifiuto e 2 in caso di errore di utilizzo o di un errore imprevisto. `--debug` consente la visualizzazione della traccia dello stack.
...
Un mondo viene ripristinato in tre modi, e nessuno di essi scrive nello stato interno del motore di fisica, motivo per cui ognuno è preciso. È possibile riprodurre gli input accettati fino a un determinato passo. È possibile copiare l'intera memoria del modulo di fisica con `imageSolver()` e ripristinarla con `restoreImage()`. Oppure, è possibile salvare l'intero "tick" con `save()` e ripristinarlo con `restore(saved)`, il che non richiede la riproduzione ed è il modo in cui la simulazione ritorna a uno stato migliaia di volte. Un'immagine proveniente da un altro binario, di lunghezza errata o con un byte modificato, viene rifiutata e un salvataggio che non verifica tutte le modifiche non cambia nulla.

La visualizzazione di debug è una visualizzazione di debug. Disegna i frame registrati come caselle proiettate lungo l'asse scelto con `x`, `y` o `z`; un clic è un target sul piano di riferimento; `M`, `C`, `G`, `D` e `U` selezionano le azioni di movimento, arrampicata, presa, rilascio e utilizzo; la zona del personaggio e le credenze di ogni entità si trovano accanto al "tick" e all'hash. Non disegna mai nulla che il "tick" non contenga.

Un file di mondo è in formato JSON: `name`, `seed`, `bodies`, `colliders`, `zones` e, facoltativamente, `heightfield` e `goal`. Un corpo è `{ id, x, y, z, vx, vy, vz, hx, hy, hz }` con un quaternione e una velocità angolare opzionali; un collider statico è una scatola definita dai suoi limiti con un quaternione opzionale attorno al suo centro; una zona è una scatola con un nome. I campi sconosciuti, gli ID duplicati, i corpi sovrapposti, un corpo all'interno di un collider, un quaternione non unitario, una zona degenerata o irraggiungibile e un obiettivo che non nomina nulla vengono tutti rifiutati con una motivazione.

## La legge, in un solo respiro

Un "tick" con un seme è la legge. Un passo, un "quantum", è di 1/64 di secondo; ogni passo viene sottoposto a hashing e l'azione di un personaggio si estende su molti passi. La riproduzione è il seme più il log di ciò che è stato ammesso. Il modello propone intenzioni, credenze tipizzate e bozze di corpo; il checker per quella classe le ammette o le rifiuta. Le bozze di azione e i file di mondo attendono fino al momento del caricamento e superano una serie di test. L'host riceve i frame registrati e restituisce le intenzioni. La presentazione non ha un percorso di ritorno all'hash.

## Modello di affidabilità

Il motore viene eseguito localmente e accede solo ai file all'interno della propria directory di checkout: mondi, bozze di azione, "fixture" e qualsiasi log che si chiede a un comando di scrivere. `host` associa `127.0.0.1`. Nessun comando apre altre socket tranne `propose`, che comunica con un server Ollama locale e con nessun altro, e solo per un ruolo il cui manifesto è stato sbloccato per agire in un mondo di prova; entrambi i ruoli dichiarati dal motore sono fissi, quindi rifiuta prima che venga caricato qualsiasi client del modello. La proposta di un modello entra nel mondo solo attraverso il "gate" del ruolo, che la vincola al manifesto del ruolo. Nessuna credenziale viene letta, archiviata o inviata. Nessun dato di telemetria viene raccolto. Il contenuto creato viene considerato non affidabile e viene convalidato al momento del caricamento; un file rifiutato non cambia nulla. Il binario WebAssembly viene compilato dal codice sorgente nel sistema di Continuous Integration (CI) e viene memorizzato tramite il suo SHA-256, non viene mai aggiunto come byte. La sua memoria è fissata a 32 MiB e non può aumentare, quindi un mondo troppo denso per esso si interrompe nello stesso modo su ogni host invece di divergere. Vedere [SECURITY.md](SECURITY.md).

## Stato del supporto

Pre-1.0, rilasciato come `0.x` da `main`. Non esiste una promessa di compatibilità tra le versioni; ogni modifica alla legge sottoposta a hashing viene registrata in [CHANGELOG.md](CHANGELOG.md) insieme all'hash di riferimento che ha prodotto. Testato su Node 22 e Rust 1.98.1 su Ubuntu x64 e ARM64 nel sistema di Continuous Integration (CI) e compilato quotidianamente su Windows 11.

## Licenza

MIT, ad eccezione di `solver/src/kcc.rs` e `solver/src/impulses.rs`, copie modificate di parti del controller di personaggi di Rapier, che sono sotto licenza Apache 2.0 (`solver/LICENSE-APACHE-2.0`, `solver/NOTICE`). Realizzato da <a href="https://mcp-tool-shop.github.io/">MCP Tool Shop</a>.
